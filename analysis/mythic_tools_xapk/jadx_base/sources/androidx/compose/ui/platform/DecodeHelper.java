...
public final class DecodeHelper {
    ...
    private final float m8235decodeBaselineShifty9eOQZs() {
    ...
    private final Shadow decodeShadow() {
        long jM8237decodeColor0d7_KjU = m8237decodeColor0d7_KjU();
        float fDecodeFloat = decodeFloat();
        return new Shadow(jM8237decodeColor0d7_KjU, Offset.m6072constructorimpl((((long) Float.floatToRawIntBits(decodeFloat())) & MuxerUtil.UNSIGNED_INT_MAX_VALUE) | (Float.floatToRawIntBits(fDecodeFloat) << 32)), decodeFloat(), null);
    }
    ...
    private final long m8236decodeULongsVKNKU() {
    ...
    public final long m8237decodeColor0d7_KjU() {
    ...
    public final int m8238decodeFontStyle_LCdwA() {
    ...
    public final int m8239decodeFontSynthesisGVVA2EU() {
        byte bDecodeByte = decodeByte();
        ...
        while (this.parcel.dataAvail() > 1) {
            ...
            if (bDecodeByte != 1) {
                if (bDecodeByte != 2) {
                    if (bDecodeByte != 3) {
                        if (bDecodeByte != 4) {
                            if (bDecodeByte != 5) {
                                        ...
                                        if (bDecodeByte != 8) {
                                            if (bDecodeByte != 9) {
                                                if (bDecodeByte != 10) {
                                                    ...
                                                    mutableSpanStyle.m8284setBackground8_81llA(m8237decodeColor0d7_KjU());
                                                }
                                            } else {
                                                if (dataAvailable() < 8) {
                                                    break;
                                            ...
                                            mutableSpanStyle.m8285setBaselineShift_isdbwI(BaselineShift.m8854boximpl(m8235decodeBaselineShifty9eOQZs()));
                                        }
                                    } else {
                                        if (dataAvailable() < 5) {
                                            break;
                                        }
                                        mutableSpanStyle.m8290setLetterSpacingR2X_6o(m8240decodeTextUnitXSAIIZE());
                                    }
                                } else {
                                ...
                                mutableSpanStyle.m8289setFontSynthesistDdu0R4(FontSynthesis.m8676boximpl(m8239decodeFontSynthesisGVVA2EU()));
                            }
                        } else {
                            if (dataAvailable() < 1) {
                                break;
                            }
                            mutableSpanStyle.m8288setFontStylemLjRB2g(FontStyle.m8665boximpl(m8238decodeFontStyle_LCdwA()));
                        }
                    } else {
                        if (dataAvailable() < 4) {
                            break;
                    ...
                    mutableSpanStyle.m8287setFontSizeR2X_6o(m8240decodeTextUnitXSAIIZE());
                }
            } else {
                if (dataAvailable() < 8) {
                    break;
                }
                mutableSpanStyle.m8286setColor8_81llA(m8237decodeColor0d7_KjU());
            }
        }
    ...
    public final long m8240decodeTextUnitXSAIIZE() {
        byte bDecodeByte = decodeByte();
