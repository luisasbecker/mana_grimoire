...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void setView(float f, MotionWidget motionWidget, int[] iArr, double[] dArr, double[] dArr2, double[] dArr3) {
        ...
        if (motion != null) {
            ...
            float fCos2 = (float) ((((double) f18) - (d4 * Math.cos(d3))) + (Math.sin(d3) * d2 * d5));
            if (dArr2.length >= 2) {
                dArr2[0] = fSin2;
                dArr2[1] = fCos2;
            }
            if (!Float.isNaN(f14)) {
                motionWidget.setRotationZ((float) (((double) f14) + Math.toDegrees(Math.atan2(fCos2, fSin2))));
            }
            f3 = fSin;
            f4 = fCos;
        } else if (!Float.isNaN(f14)) {
            motionWidget.setRotationZ(((float) (((double) f14) + Math.toDegrees(Math.atan2(f9 + (f11 / 2.0f), f8 + (f10 / 2.0f))))) + 0.0f);
        }
        float f19 = f3 + 0.5f;
        float f20 = f4 + 0.5f;
