...
public final class AesEaxJce implements Aead {
    ...
    private byte[] omac(Cipher cipher, int i, byte[] bArr, int i2, int i3) throws BadPaddingException, IllegalBlockSizeException {
        byte[] bArr2 = new byte[16];
        bArr2[15] = (byte) i;
        if (i3 == 0) {
        ...
        return cipher.doFinal(xor(bArrDoFinal, pad(Arrays.copyOfRange(bArr, i4 + i2, i2 + i3))));
    }
    ...
    public byte[] decrypt(byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ...
        byte b = 0;
        for (int i = 0; i < 16; i++) {
            b = (byte) (b | (((bArr[length2 + i] ^ bArrOmac2[i]) ^ bArrOmac[i]) ^ bArrOmac3[i]));
        }
        if (b != 0) {
    ...
    public byte[] encrypt(byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ...
        for (int i2 = 0; i2 < 16; i2++) {
            bArr3[length2 + i2] = (byte) ((bArrOmac2[i2] ^ bArrOmac[i2]) ^ bArrOmac3[i2]);
        }
        return bArr3;
    }
}
