...
class LoadingIndicatorDrawingDelegate {
    ...
    void drawContainer(Canvas canvas, Paint paint, int i, int i2) {
        float fMin = Math.min(this.specs.containerWidth, this.specs.containerHeight) / 2.0f;
        paint.setColor(MaterialColors.compositeARGBWithAlpha(i, i2));
        paint.setStyle(Paint.Style.FILL);
        canvas.drawRoundRect(new RectF((-this.specs.containerWidth) / 2.0f, (-this.specs.containerHeight) / 2.0f, this.specs.containerWidth / 2.0f, this.specs.containerHeight / 2.0f), fMin, fMin, paint);
    }
