    ...
    private static KeylineState shiftKeylineStateForPaddingUncontained(KeylineState keylineState, float f, int i, boolean z) {
        ...
        while (i2 < arrayList.size()) {
            ...
            if (keyline.isAnchor && i2 == size) {
                ...
                float f4 = f3;
                builder.addKeyline(f4, keyline.mask, keyline.maskedItemSize, i2 >= keylineState.getFirstFocalKeylineIndex() && i2 <= keylineState.getLastFocalKeylineIndex(), keyline.isAnchor, Math.abs(z ? Math.max(0.0f, ((keyline.maskedItemSize / 2.0f) + f4) - i) : Math.min(0.0f, f4 - (keyline.maskedItemSize / 2.0f))), z ? f : 0.0f, z ? 0.0f : f);
            }
            i2++;
        }
        return builder.build();
