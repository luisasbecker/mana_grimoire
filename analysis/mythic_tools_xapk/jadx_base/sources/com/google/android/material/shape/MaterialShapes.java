...
public final class MaterialShapes {
    ...
    static class VertexAndRounding {
        ...
        public void toCartesian(float f, float f2) {
            float fCos = (float) ((((double) this.vertex.y) * Math.cos(this.vertex.x)) + ((double) f));
            float fSin = (float) ((((double) this.vertex.y) * Math.sin(this.vertex.x)) + ((double) f2));
            this.vertex.x = fCos;
            this.vertex.y = fSin;
        }
        ...
        public void toRadial(float f, float f2) {
            ...
            float fAtan2 = (float) Math.atan2(this.vertex.y, this.vertex.x);
            float fHypot = (float) Math.hypot(this.vertex.x, this.vertex.y);
            this.vertex.x = fAtan2;
            this.vertex.y = fHypot;
        }
    }
