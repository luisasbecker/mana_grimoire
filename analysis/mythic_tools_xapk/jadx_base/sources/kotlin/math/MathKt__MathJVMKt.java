...
public class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final double atanh(double d) {
        return Math.abs(d) < Constants.taylor_n_bound ? Math.abs(d) > Constants.taylor_2_bound ? d + (((d * d) * d) / 3.0d) : d : Math.log((1.0d + d) / (1.0d - d)) / 2.0d;
    }
    ...
    public static final double truncate(double d) {
        return (Double.isNaN(d) || Double.isInfinite(d)) ? d : d > AudioStats.AUDIO_AMPLITUDE_NONE ? Math.floor(d) : Math.ceil(d);
    }
